...
ManaOrb::ManaOrb(){
    ...
    if(!m_shader.loadFromFile("../assets/manaOrb.vert" , "../assets/manaOrb.frag")){
        throw std::runtime_error("Failed to shader manaOrb shaders .");
    }
...
void ManaOrb::display(sf::RenderTarget & target , sf::View view){
    ...
    auto translation = m_rect.getPosition()-view.getCenter()+view.getSize()*0.5f;
    translation.x = std::round(translation.x*zoom.x);
    translation.y = std::round(translation.y*zoom.y);
    m_shader.setUniform("u_offPos" , translation);
